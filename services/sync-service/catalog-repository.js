  ...
  async function syncArtists(accessToken, artistIds, pulledArtists = []) {
    ...
    const rows = Array.from(merged.values()).map(item => {
      ...
      return {
        ...
        spotify_url: validateSpotifyUrl(item.external_urls?.spotify, 'artist') ||
          validateSpotifyUrl(previous?.spotify_url, 'artist') || null,
  ...
  async function syncAlbums(albumIds, pulledAlbums = []) {
    ...
    for (const album of albumsById.values()) {
      ...
      rows.push({
        ...
        spotify_url: validateSpotifyUrl(album.external_urls?.spotify, 'album') ||
          validateSpotifyUrl(previous?.spotify_url, 'album') || null,
  ...
  async function syncTracks(trackIds, pulledTracks = []) {
    ...
    for (const track of tracksById.values()) {
      ...
      rows.push({
        ...
        spotify_url: validateSpotifyUrl(track.external_urls?.spotify, 'track') ||
          validateSpotifyUrl(previous?.spotify_url, 'track') || null,
