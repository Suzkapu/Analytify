...
import {DESIGN_VARIANT, designPath, DesignVariant} from '@core/navigation/design-variant';
...
export const redirectLoggedInGuard = async () => {
  ...
  if (authService.isAuthenticated() && terms.hasCurrentAcceptance()) {
    router.navigateByUrl(returnUrl.consume(designPath(designVariant, 'playlists')));
    return false;
  }
