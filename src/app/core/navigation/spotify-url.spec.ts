...
describe('Spotify URL navigation', () => {
    ...
    it('rejects script, data, HTTP, credential, spoofed, encoded-host, and mismatched URLs', () => {
        ...
        blocked.forEach(url => expect(sanitizeSpotifyUrl(url), url).toBeNull());
    ...
    it('never invokes browser navigation for rejected stored values', () => {
        const fakeWindow = {
            location: { assign: vi.fn().mockName('assign') },
            open: vi.fn().mockName('open')
        };
        expect(openSpotifyUrl('javascript:alert(1)', { expectedType: 'track' }, fakeWindow)).toBe(false);
        expect(fakeWindow.location.assign).not.toHaveBeenCalled();
        expect(fakeWindow.open).not.toHaveBeenCalled();
        expect(openSpotifyUrl('https://open.spotify.com/track/abc', { expectedType: 'track' }, fakeWindow)).toBe(true);
        expect(fakeWindow.open).toHaveBeenCalledWith('https://open.spotify.com/track/abc', '_blank', 'noopener,noreferrer');
    });
});
