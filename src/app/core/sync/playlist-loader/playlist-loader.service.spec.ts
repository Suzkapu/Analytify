...
import { buildPlaylistSourceManifest, sourceEntriesFromSpotify } from './playlist-source-manifest';
...
describe('PlaylistLoaderService', () => {
    ...
    beforeEach(() => {
        storageValues = {};
        spotify = {
            getFavTracks: vi.fn().mockName("SpotifyDataService.getFavTracks"),
            getSinglePlaylist: vi.fn().mockName("SpotifyDataService.getSinglePlaylist"),
            getAllTracksFromPlaylist: vi.fn().mockName("SpotifyDataService.getAllTracksFromPlaylist"),
            getPlaylistMetadata: vi.fn().mockName("SpotifyDataService.getPlaylistMetadata")
        };
    ...
    it('rejects a severely incomplete legacy playlist cache', () => {
        ...
        expect(service.isCachedPlaylistComplete(artists, 5000, null)).toBe(false);
    });
    ...
    it('accepts a completed cache whose consistency marker matches its data', () => {
        ...
        expect(service.isCachedPlaylistComplete(artists, 5000, 4975)).toBe(true);
    });
    ...
    it('keeps the cautious source-total tolerance for legacy caches without a marker', () => {
        ...
        expect(service.isCachedPlaylistComplete(artists, 5000, null)).toBe(true);
    });

    it('does not invalidate track data merely because artist images are absent', () => {
        const artists = [{ id: 'artist', tracks: [{ id: 'track' }] }];

        expect(service.isCachedPlaylistComplete(artists, 1, 1)).toBe(true);
    });
    ...
    it('trusts an exact completed-cache marker when Spotify includes duplicate or unavailable entries', () => {
        ...
        expect(service.isCachedPlaylistComplete(artists, 100, 80)).toBe(true);
    });

    it('does not accept an empty marked cache when Spotify reports playlist entries', () => {
        expect(service.isCachedPlaylistComplete([], 10, 0)).toBe(false);
    });
    ...
    it('rejects a cache whose consistency marker does not match its data', () => {
        ...
        expect(service.isCachedPlaylistComplete(artists, 5000, 5000)).toBe(false);
    });
    ...
    it('publishes the cached total while the first Spotify page is pending', () => {
        ...
        spotify.getSinglePlaylist.mockReturnValue(NEVER);
        ...
        expect(task.progress$.value.isLoadingTracks).toBe(true);
    });

    it('fetches remaining playlist pages with four-way bounded concurrency', () => {
        const pages = new Map([50, 100, 150, 200, 250].map(offset => [offset, new Subject<any>()]));
        spotify.getAllTracksFromPlaylist.mockImplementation((_playlistId: string, offset: number) => pages.get(offset)!.asObservable());
    ...
    it('reuses the active task instead of starting a duplicate Spotify load', () => {
        const trigger = vi.spyOn(service as any, 'triggerApiLoad').mockImplementation(() => {
        });
    ...
    it('publishes and persists complete tracks before optional artist enrichment finishes', () => {
        ...
        const persist = vi.spyOn(service as any, 'setSessionStorage').mockImplementation(() => {
        });
        vi.spyOn(service as any, 'hydrateArtistDetailsFromSupabase').mockReturnValue(new Promise<void>(() => { }));

        (service as any).finishTrackLoading(task, artists, [], 'user');

        expect(task.artists).toBe(artists);
        expect(task.isLoadingTracks).toBe(false);
        expect(task.isRefreshing).toBe(false);
        expect(task.hasDataChanges).toBe(true);
    ...
    it('soft-pulls only a new Liked Songs prefix when the raw totals reconcile', async () => {
        ...
        spotify.getFavTracks.mockReturnValue(of({ total: remoteItems.length, items: remoteItems }));

        const task = service.startNewFavouriteTracksCheck('user', true)!;
        await completed(task);

        expect(spotify.getFavTracks).toHaveBeenCalledTimes(1);

        expect(spotify.getFavTracks).toHaveBeenCalledWith(0, 50);
        ...
        expect(JSON.parse(storageValues['user_fav_SourceState']).dirty).toBe(false);
    });
    ...
    it('keeps the previous cache when a playlist snapshot changes during pagination', async () => {
        ...
        spotify.getSinglePlaylist.mockReturnValue(of({
            id: 'playlist',
            name: 'Playlist',
            snapshot_id: 'snapshot-before',
            tracks: { total: 2, items: [first] }
        }));
        spotify.getAllTracksFromPlaylist.mockReturnValue(of({ total: 2, items: [second] }));
        spotify.getPlaylistMetadata.mockReturnValue(of({
    ...
    it('marks a changed playlist snapshot dirty and trusts an unchanged snapshot', () => {
        ...
        expect(service.isPlaylistSourceDirty('user', 'playlist')).toBe(false);
        ...
        expect(service.isPlaylistSourceDirty('user', 'playlist')).toBe(true);
    });

    function seedLikedSongs(ids: string[]): void {
        const entries = ids.map((id, index) => spotifyEntry(id, `2026-08-${String(ids.length - index).padStart(2, '0')}T00:00:00Z`));
    ...
    function sourceManifestJson(playlistId: string, entries: any[], snapshotId: string | null = null): string {
        const manifest = buildPlaylistSourceManifest(playlistId, entries.length, sourceEntriesFromSpotify(entries), new Set(entries.map(entry => entry.track?.id).filter(Boolean)).size, snapshotId);
        if (!manifest)
            throw new Error('Test manifest could not be created.');
    ...
    async function completed(task: PlaylistLoadTask): Promise<any> {
        return firstValueFrom(task.progress$.pipe(filter(progress => progress.isComplete), take(1)));
    }
});
