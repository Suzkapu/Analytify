...
describe('async loading primitives', () => {
    ...
    it('defers background work instead of running it in the critical turn', () => {
        vi.useFakeTimers();
        const task = vi.fn().mockName('task');
    ...
    it('serializes writes for one key without blocking independent keys', async () => {
        ...
        releases.shift()?.();
        await first;
        await vi.waitFor(() => expect(started).toEqual(['first', 'independent', 'second']));
        releases.shift()?.();
    ...
    it('continues a keyed queue after an earlier write fails', async () => {
        ...
        await expect(failed).rejects.toThrowError('conflict');
        await expect(recovered).resolves.toEqual('saved');
    });
});
