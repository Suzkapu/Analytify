...
describe('PlaylistsComponent', () => {
    ...
    beforeEach(() => {
        ...
        spotifyDataService = {
            getAccessibleUserPlaylists: vi.fn().mockName("SpotifyDataService.getAccessibleUserPlaylists"),
        ...
        authService.getUserId.mockReturnValue('current-user');
        authService.isBackupActive.mockReturnValue(false);
        authService.isAuthenticated.mockReturnValue(false);
        authService.isPersonalAppConnection.mockReturnValue(false);
        authService.getAccessToken.mockReturnValue('access-token');
        authService.isTokenExpired.mockReturnValue(false);
        storageService.getItem.mockImplementation((key: string) => storage.get(key) ?? null);
        storageService.setItem.mockImplementation((key: string, value: string) => storage.set(key, value));
        storageService.restoreItemsFromCloud.mockResolvedValue(0);
    ...
    it('shows loading instead of a false empty-playlists result on first paint', () => {
        ...
        expect(component.isLoadingPlaylists).toBe(true);
    ...
    it('does not render sharing controls on individual playlist cards', () => {
        authService.isBackupActive.mockReturnValue(true);
    ...
    it('uses a fresh cloud playlist portfolio before falling back to Spotify', async () => {
        authService.isBackupActive.mockReturnValue(true);
        storageService.restoreItemsFromCloud.mockImplementation(async () => {
            storage.set('current-user_playlists', JSON.stringify([
    ...
    it('reuses a Liked Songs count that was refreshed after the daily cutoff', async () => {
        ...
        spotifyDataService.getAccessibleUserPlaylists.mockReturnValue(of({
    ...
    it('loads the public profile ID before classifying personal-app playlist owners', async () => {
        authService.isPersonalAppConnection.mockReturnValue(true);
        spotifyDataService.getCurrentUser.mockReturnValue(of({ id: 'public-profile-id' }));
        spotifyDataService.getAccessibleUserPlaylists.mockReturnValue(of({
            currentUserId: 'public-profile-id',
            items: [{ id: 'owned', owner: { id: 'public-profile-id' }, items: { total: 2 } }]
        }));
        spotifyDataService.getFavTracks.mockReturnValue(of({ total: 0 }));
