alter table public.sync_user_settings
  add column if not exists history_interval_unit text not null default 'minutes',
  add column if not exists short_term_interval_unit text not null default 'hours',
  add column if not exists medium_term_interval_unit text not null default 'hours',
  add column if not exists long_term_interval_unit text not null default 'hours',
  add column if not exists song_league_playlist_interval_unit text not null default 'minutes',
  add column if not exists shared_playlist_interval_unit text not null default 'minutes';

alter table public.sync_user_settings
  add constraint sync_user_settings_history_interval_unit_check check (history_interval_unit in ('minutes', 'hours', 'days')),
  add constraint sync_user_settings_short_term_interval_unit_check check (short_term_interval_unit in ('minutes', 'hours', 'days')),
  add constraint sync_user_settings_medium_term_interval_unit_check check (medium_term_interval_unit in ('minutes', 'hours', 'days')),
  add constraint sync_user_settings_long_term_interval_unit_check check (long_term_interval_unit in ('minutes', 'hours', 'days')),
  add constraint sync_user_settings_song_league_playlist_interval_unit_check check (song_league_playlist_interval_unit in ('minutes', 'hours', 'days')),
  add constraint sync_user_settings_shared_playlist_interval_unit_check check (shared_playlist_interval_unit in ('minutes', 'hours', 'days'));

alter table public.sync_user_settings drop constraint if exists sync_user_settings_history_interval_minutes_check;
alter table public.sync_user_settings drop constraint if exists sync_user_settings_short_term_interval_hours_check;
alter table public.sync_user_settings drop constraint if exists sync_user_settings_medium_term_interval_hours_check;
alter table public.sync_user_settings drop constraint if exists sync_user_settings_long_term_interval_hours_check;
alter table public.sync_user_settings drop constraint if exists sync_user_settings_song_league_playlist_interval_minutes_check;
alter table public.sync_user_settings drop constraint if exists sync_user_settings_shared_playlist_interval_minutes_check;
alter table public.sync_user_settings
  add constraint sync_user_settings_history_interval_value_check check (history_interval_minutes between 1 and 10080),
  add constraint sync_user_settings_short_term_interval_value_check check (short_term_interval_hours between 1 and 10080),
  add constraint sync_user_settings_medium_term_interval_value_check check (medium_term_interval_hours between 1 and 10080),
  add constraint sync_user_settings_long_term_interval_value_check check (long_term_interval_hours between 1 and 10080),
  add constraint sync_user_settings_song_league_playlist_interval_value_check check (song_league_playlist_interval_minutes between 1 and 10080),
  add constraint sync_user_settings_shared_playlist_interval_value_check check (shared_playlist_interval_minutes between 1 and 10080);
...
returns table (
  ...
  song_league_playlists_enabled boolean, song_league_playlist_interval_minutes integer, song_league_playlist_interval_unit text,
...
begin
  ...
  select
    ...
    coalesce(settings.song_league_playlists_enabled, true), coalesce(settings.song_league_playlist_interval_minutes, 60),
...
drop function if exists public.admin_update_sync_user(uuid, boolean, text, boolean, integer, boolean, integer, boolean, integer, boolean, integer, boolean, integer, boolean, integer);
create function public.admin_update_sync_user(
  ...
  p_song_league_playlists_enabled boolean, p_song_league_playlist_interval_minutes integer, p_song_league_playlist_interval_unit text,
...
begin
  ...
  insert into public.sync_user_settings(
    ...
    song_league_playlists_enabled = excluded.song_league_playlists_enabled, song_league_playlist_interval_minutes = excluded.song_league_playlist_interval_minutes, song_league_playlist_interval_unit = excluded.song_league_playlist_interval_unit,
