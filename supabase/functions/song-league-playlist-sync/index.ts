...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    let finalResults: Array<{userId: string; success: boolean; error?: string}> = [];
    let finalRevision = -1;
    ...
    for (let pass = 0; pass < 3; pass++) {
      ...
      const targetMembers = (members || []).filter((member: any) =>
        createForCurrentUser
          ? member.user_id === identity.user.id
          : mappingById.has(member.user_id)
      );
      ...
      for (const member of targetMembers) {
        ...
        try {
          ...
          finalResults.push({userId: member.user_id, success: true});
        } catch (error) {
          ...
          finalResults.push({userId: member.user_id, success: false, error: message});
        }
      }
    ...
    return json({
      ...
      synced: finalResults.length - failed.length,
